...
(setq auto-save-file-name-transforms '((".*" "~/.local/share/emacs/auto-save-list/" t)) ) ;transform backups file name
...
(defun rpc/open-in-github ()
  ...
  (let* ((branch (magit-get-current-branch))
		 (rev (magit-rev-parse branch))
		 (remote (magit-get-push-remote branch))
		 (url (magit-get (format "remote.%s.url" remote)))
		 (_ (string-match "git@github.com:\\(.+\\)/\\(.+\\)" url))
		 (org (match-string-no-properties 1 url))
		 (repo (match-string-no-properties 2 url))
